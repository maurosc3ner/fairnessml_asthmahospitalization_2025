...
v1
bootstrapping for glm models focused on length of stay
extensive code clean-up for equations 
factors based of los.fairness.univariate.v1
...
```{r}
hist(data.splitted.los[["iter1"]]$data2.train$los,64)

summary(data.splitted.los[["iter1"]]$data2.train$los)
```

```{r}
rb.eq<-formula(losBinaryNum ~
      s.age:sex_at_birth +
      s.dep +
      race3 +
      hcType1 +
...
# for(i in 1:2){
  ...
  model.raa.glm.ss <- glmer(
    formula = rs.eq,
    data = data.splitted.los[[idx]]$data2.train.AA,
    family = binomial(),
    nAGQ = 0,
    glmerControl(
      optimizer = "bobyqa",
  ...
  model.rea.glm.ss <- glmer(
    formula = rs.eq,
    data = data.splitted.los[[idx]]$data2.train.EA,
    family = binomial(),
    # nAGQ = 0,
    glmerControl(
      optimizer = "bobyqa",
  ...
  model.ras.glm.ss <- glmer(
    formula = rs.eq,
    data = data.splitted.los[[idx]]$data2.train.AS,
    family = binomial(),
    # nAGQ = 0,
    glmerControl(
      optimizer = "bobyqa",
  ...
  model.rhl.glm.ss <- glmer(
    formula = rs.eq,
    data = data.splitted.los[[idx]]$data2.train.HL,
    family = binomial(),
    # nAGQ = 0,
    glmerControl(
      optimizer = "bobyqa",
  ...
  model.rmena.glm.ss <- glmer(
    formula = rs.eq,
    data = data.splitted.los[[idx]]$data2.train.MENA,
    family = binomial(),
    # nAGQ = 0,
    glmerControl(
      optimizer = "bobyqa",
  ...
  ## for rb, rn
  # data.splitted.los[[idx]]$data2.valid$hcType1<-factor(data.splitted.los[[idx]]$data2.valid$hcType1)
  dfResults=data.splitted.los[[idx]]$data2.valid %>% dplyr::select(race3,losBinaryFactor)
  dfResults$prob.rb.glm<-predict(model.rb.glm.ss,data.splitted.los[[idx]]$data2.valid,type='response',allow.new.levels=T)
  dfResults$r30d.pred.rb.glm.ss <- factor(ifelse(dfResults$prob.rb.glm>0.5,myLevels[1],myLevels[2]),levels=myLevels)
  dfResults$prob.rn.glm<-predict(model.rn.glm.ss,data.splitted.los[[idx]]$data2.valid,type='response',allow.new.levels=T)
  ...
  dfResults.aa=data.splitted.los[[idx]]$data2.valid.AA %>% dplyr::select(race3,losBinaryFactor)
  dfResults.aa$prob.raa.glm<-predict(model.raa.glm.ss,data.splitted.los[[idx]]$data2.valid.AA,type='response',allow.new.levels=T)
  dfResults.aa$r30d.pred.raa.glm.ss <- factor(ifelse(dfResults.aa$prob.raa.glm>0.5,myLevels[1],myLevels[2]),levels=myLevels)
  dfResults.ea=data.splitted.los[[idx]]$data2.valid.EA %>% dplyr::select(race3,losBinaryFactor)
  dfResults.ea$prob.rea.glm<-predict(model.rea.glm.ss,data.splitted.los[[idx]]$data2.valid.EA,type='response',allow.new.levels=T)
  dfResults.ea$r30d.pred.rea.glm.ss <- factor(ifelse(dfResults.ea$prob.rea.glm>0.5,myLevels[1],myLevels[2]),levels=myLevels)
  dfResults.as=data.splitted.los[[idx]]$data2.valid.AS %>% dplyr::select(race3,losBinaryFactor)
  dfResults.as$prob.ras.glm<-predict(model.ras.glm.ss,data.splitted.los[[idx]]$data2.valid.AS,type='response',allow.new.levels=T)
  dfResults.as$r30d.pred.ras.glm.ss <- factor(ifelse(dfResults.as$prob.ras.glm>0.5,myLevels[1],myLevels[2]),levels=myLevels)
  dfResults.hl=data.splitted.los[[idx]]$data2.valid.HL %>% dplyr::select(race3,losBinaryFactor)
  dfResults.hl$prob.rhl.glm<-predict(model.rhl.glm.ss,data.splitted.los[[idx]]$data2.valid.HL,type='response',allow.new.levels=T)
  dfResults.hl$r30d.pred.rhl.glm.ss <- factor(ifelse(dfResults.hl$prob.rhl.glm>0.5,myLevels[1],myLevels[2]),levels=myLevels)
  dfResults.mena=data.splitted.los[[idx]]$data2.valid.MENA %>% dplyr::select(race3,losBinaryFactor)
  dfResults.mena$prob.rmena.glm<-predict(model.rmena.glm.ss,data.splitted.los[[idx]]$data2.valid.MENA,type='response',allow.new.levels=T)
  ...
  cm<-confusionMatrix(
    ...
    reference=dfResults$losBinaryFactor)
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
                      fairmetrics2DF(F,"All",model1Label,cm))
  # print(cm)
  # rb black aa
  cm<-confusionMatrix(
    data = dfResults %>% filter(race3 == 'Black_AA') %>% pull(r30d.pred.rb.glm.ss), 
    reference = dfResults %>% filter(race3 == 'Black_AA') %>% pull(losBinaryFactor))
  # print(cm$table)
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
  ...
  cm<-confusionMatrix(
    ...
    reference = dfResults %>% filter(race3 == 'White_EA') %>% pull(losBinaryFactor))
  # print(cm$table)
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
  ...
  cm<-confusionMatrix(
    ...
    reference = dfResults %>% filter(race3 == 'Asian') %>% pull(losBinaryFactor))
  # print(cm$table)
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
  ...
  cm<-confusionMatrix(
    ...
    reference = dfResults %>% filter(race3 == 'HL') %>% pull(losBinaryFactor))
  # print(cm$table)
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
  ...
  cm<-confusionMatrix(
    ...
    reference = dfResults %>% filter(race3 == 'MENA') %>% pull(losBinaryFactor))
  testDF.los.glm.bt<-rbind(testDF.los.glm.bt,
...
# for(i in 1:2){
  ...
  if(upsampling){
    # databup<-data.splitted.los[[idx]]$data2.train
    # databup$race4=paste0(data.splitted.los[[idx]]$data2.train$race3,
    #                      data.splitted.los[[idx]]$data2.train$losBinaryFactor)
    data=upSampler(data.splitted.los[[idx]]$data2.train,"race3",0.7)
  }else{
    data=data.splitted.los[[idx]]$data2.train
  }
...
```{r}
save(testDF.los.glm.bt,file = "testDF.los.glm.bt.RData",compress = T)
```
